...
interface AttachmentViewerProps {
  ...
  canUpload?: boolean;
  uploadFunction:
    | ((entityId: string, file: File) => Promise<any>)
...
export default function AttachmentViewer({
  ...
  const uploadMut = useMutation({
    mutationFn: async (file: File) => {
      ...
      if (entityType === "temp") {
        return (uploadFunction as any)(file);
      }
      return (uploadFunction as any)(entityId, file);
    },
    onSuccess: () => {
      if (entityType !== "temp") {
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      push(
        "File type not supported. Please upload PDF, images, documents, or text files.",
        "error"
      );
      return;
    }
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
      ...
      {canUpload && (
        <div
          ...
          } text-center mb-4 transition-all duration-200 ${
            isDragOver
        ...
        >
          <i
            className={`fas fa-cloud-upload-alt ${
              compact ? "text-xl mb-1" : "text-2xl sm:text-3xl mb-2"
            } transition-colors ${
          ...
          <p className="text-xs text-gray-500 mt-1">
            <span className="hidden sm:inline">
              Supports: PDF, Images, Word, Excel, Text files (Max 10MB)
            </span>
      ...
      {attachments.length > 0 ? (
        <div className="space-y-3">
          {attachments.map((attachment: any) => (
            <div
              key={attachment.id}
              className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors border border-gray-200"
            >
              ...
              <div className="flex flex-col sm:flex-row items-end sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0">
                {entityType === "temp" ? (
                  <button
                    onClick={() => {
                      // For temporary files, create a preview URL
                      if (attachment.file) {
                ...
                {entityType === "temp" && onDelete && (
                  <button
                    onClick={() => onDelete(attachment.id)}
                    className="inline-flex items-center px-2 sm:px-3 py-1.5 text-xs font-medium text-red-600 bg-red-100 rounded-md hover:bg-red-200 transition-colors"
                  >
        ...
        <div className="text-center py-8 sm:py-12 text-gray-500">
          ...
          <p className="text-sm">
            {canUpload
              ? "Upload supporting documents to get started."
