...
export default function ApprovalsPage() {
  ...
  const handleModalSubmit = (
    ...
    switch (action) {
      case "approve":
        approveMutation.mutate({ id: request.id, reason: reason || "" });
        break;
      case "reject":
        if (!reason) return;
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            This area is reserved for Approvers and Finance Officers only.
          </p>
          <p className="text-sm text-gray-500">
            Contact your administrator if you need access to approval functions.
          </p>
        </div>
      </div>
    );
  ...
  const submittedRequests =
    pendingRequests?.filter((r) => r.status === "SUBMITTED") || [];
  const revisionRequests =
    pendingRequests?.filter((r) => r.status === "NEEDS_REVISION") || [];
  ...
  const getRoleInfo = () => {
    if (currentUser?.role === "FINANCE_OFFICER") {
      return {
        ...
        description:
          "Review service requests, manage approvals, and oversee financial operations.",
      };
    }
    return {
      title: "Approval Dashboard",
      description:
        "Review and approve service requests submitted by restaurant staff.",
    };
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {totalPending === 0 ? (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center border border-gray-200">
            ...
            <p className="text-sm text-gray-500">
              All requests have been processed or there are no submitted
              requests yet.
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                          ...
                          <p className="text-sm font-semibold text-gray-900">
                            {formatGhanaDate(request.eventDate)}
                          </p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                          <p className="text-xs font-medium text-gray-500 mb-1">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-900">
                            {formatGhanaDate(request.createdAt)}
                          </p>
                        </div>
                      </div>
                    </div>
