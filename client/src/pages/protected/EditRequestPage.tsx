...
const FUNDING_SOURCE_OPTIONS = [
  { value: "departmental_budget", label: "Departmental Budget" },
  { value: "project_funds", label: "Project Funds" },
  { value: "external_grant", label: "External Grant" },
  { value: "student_fees", label: "Student Fees" },
  { value: "university_funds", label: "University Funds" },
...
function splitOrderDescription(description?: string | null) {
  ...
  const [notesPart, ...summaryParts] = safeDescription.split(
    `${ORDER_SUMMARY_MARKER}`,
  );
  return {
...
export default function EditRequestPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["request", id],
    queryFn: async () => getRequestById(id!) as Promise<ServiceRequest>,
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (data) {
      ...
      setOrderSummary(parsedDescription.orderSummary);
      setIsOrderRequest(parsedDescription.isOrderRequest);
  ...
  useEffect(() => {
    const pricePerPerson = Number(formData.pricePerPerson || 0);
    const attendees = Number(formData.attendees || 0);

    if (pricePerPerson > 0 && attendees > 0) {
      const nextEstimate = Number((pricePerPerson * attendees).toFixed(2));
      if (Number(formData.estimateAmount || 0) !== nextEstimate) {
        setFormData((prev) => ({ ...prev, estimateAmount: nextEstimate }));
      }
    }
  }, [formData.attendees, formData.pricePerPerson, formData.estimateAmount]);
  ...
  const uploadFiles = async () => {
    ...
    setUploading(true);
    try {
      await Promise.all(files.map((file) => uploadRequestAttachment(id, file)));
  ...
  const mutation = useMutation({
    mutationFn: (payload: UpdateServiceRequestInput) =>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const trimmedOrderSummary = orderSummary.trim();
    const mergedDescription =
      isOrderRequest && trimmedOrderSummary
        ? notes
          ? `${notes}\n\n${ORDER_SUMMARY_MARKER}\n${trimmedOrderSummary}`
          : `${ORDER_SUMMARY_MARKER}\n${trimmedOrderSummary}`
        : notes;
    ...
    const submitData = {
      ...
      pricePerPerson: formData.pricePerPerson
        ...
        : undefined,
      selectedPackageId: formData.selectedPackageId || undefined,
  ...
  if (error)
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center text-red-600">Failed to load request</div>
      </div>
    );
  if (!data)
    return (
  ...
  const canEdit = ["SUBMITTED", "NEEDS_REVISION", "DRAFT"].includes(
    data.status,
  );
  const hasCustomServiceType =
    !!formData.serviceType &&
    !SERVICE_TYPE_OPTIONS.includes(formData.serviceType);
  const hasCustomFundingSource =
    !!formData.fundingSource &&
    !FUNDING_SOURCE_OPTIONS.some(
      (option) => option.value === formData.fundingSource,
    );
  const isPackagePricedRequest = Number(formData.pricePerPerson || 0) > 0;
        ...
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
            ...
            <p className="text-gray-600 mb-4">
              ...
              <strong>{data.status.toLowerCase().replace("_", " ")}</strong> and
              cannot be modified.
              {data.status === "APPROVED" &&
                " Once approved, requests can only be edited by administrators."}
              {data.status === "REJECTED" &&
                " Rejected requests cannot be edited. Please create a new request if needed."}
              {data.status === "FULFILLED" &&
                " Fulfilled requests are finalized and cannot be changed."}
              {data.status === "CLOSED" &&
                " Closed requests are archived and cannot be modified."}
            </p>
      ...
      <div className="form-container-compact">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <div className="form-grid">
              ...
              <div className="form-field">
                ...
                >
                  ...
                  {hasCustomServiceType && (
                    <option value={formData.serviceType || ""}>
                      {formData.serviceType} (existing order service type)
                    </option>
                  )}
                </select>
              </div>

              {isPackagePricedRequest && (
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="grid gap-2 text-sm">
                      <p className="text-gray-800">
                        ...
                        {formData.packageName || "Selected package"}
                      </p>
                      <p className="text-gray-800">
                        <span className="font-medium">Price per person:</span> ₵
                        {Number(formData.pricePerPerson).toFixed(2)}
                      </p>
                      <p className="text-blue-700 font-medium">
                        Auto calculation: ₵
                        {Number(formData.pricePerPerson).toFixed(2)} x{" "}
                        {Number(formData.attendees || 0)} attendees = ₵
                        {Number(formData.estimateAmount || 0).toFixed(2)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {isOrderRequest && orderSummary && (
              <div className="form-field">
                <label className="form-label">Ordered Items Summary</label>
                <textarea
                  value={orderSummary}
                  readOnly
                  rows={6}
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Ordered items are preserved from checkout. Update event
                  details here; item lines stay consistent with the original
                  order summary.
                </p>
              </div>
            )}
            ...
            <div className="form-grid">
              <div className="form-field">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    readOnly={isPackagePricedRequest}
                  />
                </div>
                {isPackagePricedRequest && (
                  <p className="text-sm text-green-600 mt-2">
                    <i className="fas fa-calculator mr-1"></i>
                    Estimate is calculated from package price and attendees.
                  </p>
                )}
              </div>
              ...
              <div className="form-field">
                ...
                >
                  ...
                  {hasCustomFundingSource && (
                    <option value={formData.fundingSource || ""}>
                      {formData.fundingSource} (existing value)
                    </option>
                  )}
                </select>
              </div>
          ...
          <div>
                ...
                {files.map((file, index) => (
                  ...
                  >
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      <i
                        className={`${getFileIcon(file.name, file.type)} text-xl`}
                      />
            ...
            <div className="mt-3 text-sm text-gray-600">
              ...
              Upload any additional documents. Existing attachments are managed
