...
import { getRequests, getInvoices, getPayments } from '../../services/request.service';
...
export default function FinanceDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'requests' | 'finance' | 'users'>('overview');
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <h1 className="text-xl font-semibold text-gray-900 mb-2">Finance Officer Access Required</h1>
          <p className="text-gray-600 mb-4">This dashboard is reserved for Finance Officers only.</p>
          <p className="text-sm text-gray-500">Contact your administrator if you need access to finance functions.</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div className="flex-1">
              ...
              <p className="text-gray-600 max-w-3xl">
                Comprehensive financial management and oversight for restaurant operations.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex overflow-x-auto space-x-4 sm:space-x-8" aria-label="Tabs">
            {[
              ...
              { key: 'requests', label: 'Requests', iconShort: 'Req', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
              { key: 'finance', label: 'Finance', iconShort: 'Fin', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1' },
              ...
              <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">GHS {Number(stats.totalRevenue || 0).toFixed(2)}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{stats.totalInvoices} total</p>
                  </div>
                </Link>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{stats.totalPayments} recorded</p>
                  </div>
                </Link>
                  ...
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full mr-3 ${
                        activity.type === 'request' ? 'bg-primary-400' : 'bg-green-400'
                      }`}></div>
                      <div>
                    ...
                    <div className="status-wrapper">
                      <span className={`status-badge ${
                        activity.status === 'completed' ? 'status-fulfilled' :
                        activity.status === 'SUBMITTED' ? 'status-submitted' :
                        activity.status === 'APPROVED' ? 'status-approved' :
                        activity.status === 'REJECTED' ? 'status-rejected' :
                        'status-draft'
                      }`}>
                        <i className={`${
                          activity.status === 'completed' ? 'fas fa-check-double' :
                          activity.status === 'SUBMITTED' ? 'fas fa-clock' :
                          activity.status === 'APPROVED' ? 'fas fa-check-circle' :
                          activity.status === 'REJECTED' ? 'fas fa-times-circle' :
                          'fas fa-file-alt'
                        } mr-1`}></i>
        ...
        {activeTab === 'requests' && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-medium text-gray-900">Requests & Approvals Management</h3>
              <Link
                to="/approvals"
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Use the dedicated Approval Dashboard to review, approve, reject, or request revisions for service requests.
              </p>
              <p className="text-sm text-gray-500">
                You have {stats.pendingApprovals} requests waiting for your review.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="text-center py-4">
                  ...
                  <p className="text-2xl font-semibold text-gray-900 mb-1">{stats.totalInvoices}</p>
                  <p className="text-sm text-gray-600">Total Invoices Created</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="text-center py-4">
                  ...
                  <p className="text-2xl font-semibold text-gray-900 mb-1">GHS {Number(stats.totalRevenue || 0).toFixed(2)}</p>
        ...
        {activeTab === 'users' && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Manage user roles and permissions across the restaurant management system.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-lg mx-auto">
                ...
                <div className="text-center">
                  <p className="text-2xl font-semibold text-gray-900">
                    {users?.filter((u: any) => u.role === 'APPROVER').length || 0}
                  </p>
                  <p className="text-sm text-gray-600">Approvers</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-semibold text-gray-900">
                    {users?.filter((u: any) => u.role === 'FINANCE_OFFICER').length || 0}
                  </p>
