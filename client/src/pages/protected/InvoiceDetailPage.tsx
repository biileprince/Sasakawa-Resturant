      ...
      <div className="max-w-5xl mx-auto p-6">
        <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
          ...
          <p className="text-gray-600">
            You can only view invoices related to your own requests.
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <nav className="text-xs text-gray-500 flex flex-wrap items-center gap-1">
            ...
            <span className="text-gray-700 font-mono">
              #{data.invoiceNo || data.id.slice(0, 8)}
            </span>
          </nav>
          <Link
            to="/invoices"
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          <Stat
            ...
            value={Intl.NumberFormat(undefined, {
              style: "currency",
              currency: "GHS",
            }).format(Number(data.netAmount) || 0)}
          />
          <Stat label="Status" value={<StatusBadge status={data.status} />} />
          <Stat
            label="Due Date"
            value={data.dueDate ? formatGhanaDate(data.dueDate) : "-"}
          />
                      ...
                      >
                        ...
                        <td className="p-3">
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-700 border">
                            <i
                              className={`fas ${getPaymentMethodIcon(
                                payment.method
                              )}`}
                            ></i>
                            {payment.method}
        ...
        <section className="bg-white rounded-lg shadow-sm border p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <Info
                label="Gross Amount"
                value={`₵${Number(data.grossAmount || 0).toLocaleString()}`}
              />
              <Info
                label="Tax Amount"
                value={`₵${Number(data.taxAmount || 0).toLocaleString()}`}
              />
              <Info
                label="Net Amount"
                value={`₵${Number(data.netAmount || 0).toLocaleString()}`}
              />
              <Info
                label="Status"
              ...
              <Info
                ...
                value={
                  <span className="font-mono text-gray-900 bg-gray-100 px-2 py-1 rounded">
                    {data.invoiceNo || data.id}
                  </span>
                }
              />
              <Info
                label="Due Date"
                value={data.dueDate ? formatGhanaDate(data.dueDate) : "Not set"}
              />
