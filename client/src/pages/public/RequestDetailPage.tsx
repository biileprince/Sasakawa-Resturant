...
export default function RequestDetailPage() {
  ...
  const mkMut = (fn: (id: string) => Promise<any>) =>
    useMutation({
      mutationFn: () => fn(id!),
      onSuccess: () => {
        push("Status updated");
  ...
  const revisionMut = useMutation({
    mutationFn: async (comments: string) => {
      const response = await apiClient.post(`/requests/${id}/revision`, {
        comments,
      });
      return response.data;
    },
  ...
  const canEdit =
    ...
    ["SUBMITTED", "NEEDS_REVISION", "DRAFT"].includes(data.status);
  const canApprove =
    user?.capabilities?.canApproveRequest &&
  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:py-8">
      ...
      <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-6">
        <Link
          to="/"
          className="link link-internal hover:text-green-600 transition-colors"
        >
        ...
        <span className="text-gray-700">
          #{data.requestNo || data.id.slice(0, 8)}
        </span>
      </nav>
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-sm text-gray-500">
                    Request #{data.requestNo || data.id.slice(0, 8)}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            {canEdit && (
              <Link
                to={`/requests/${id}/edit`}
                className="btn-action btn-action-edit flex-1 sm:flex-none whitespace-nowrap"
              >
                ...
                {!modalState.isOpen && (
                  <>
                    ...
                    {data.status === "SUBMITTED" && (
                      <button
                        onClick={() =>
                          setModalState({ isOpen: true, action: "revision" })
                        }
      ...
      <ApprovalModal
        ...
        isSubmitting={
          approveMut.isPending || rejectMut.isPending || revisionMut.isPending
        }
      />
          ...
          <div className="dashboard-card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        {data.pricePerPerson && (
                          <div className="mt-2 flex items-center justify-between">
                            ...
                            <span className="font-semibold text-primary-600">
                              ₵{Number(data.pricePerPerson).toFixed(2)}
                            </span>
                          </div>
                        )}
                        {data.pricePerPerson && data.attendees && (
                          <div className="mt-1 text-xs text-gray-500 bg-white px-2 py-1 rounded">
                            <i className="fas fa-calculator mr-1"></i>
                            Calculation: ₵
                            {Number(data.pricePerPerson).toFixed(2)} ×{" "}
                            {data.attendees} attendees = ₵
                            {(
                              Number(data.pricePerPerson) * data.attendees
                            ).toFixed(2)}
                          </div>
                        )}
                      </div>
              ...
              <DetailItem
                ...
                label="Estimate Amount (GHS)"
                value={`₵${
                  data.estimateAmount
                    ? Number(data.estimateAmount).toFixed(2)
                    : "0.00"
                }`}
                highlight={true}
              />
                    ...
                    {data.invoices.map((inv: any) => (
                      ...
                      >
                        <td className="py-3 px-4">
                          <Link
                            to={`/invoices/${inv.id}`}
                            className="link link-internal font-medium"
                          >
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <Link
                          to={`/invoices/${inv.id}`}
                          className="link link-internal font-medium text-sm"
                        >
                          #{inv.invoiceNo || inv.id.slice(0, 8)}
                        </Link>
                      </div>
